...
def random_rotate(image):
    ...
    # label = ndimage.rotate(label, angle, order=0, reshape=False, axes=(0, 1))
    return image
...
def random_zoom(image):
    ...
    # label = zoom(label, (ratio, ratio), order=0)
    ...
    if new_img_shape > img_shape:
        ...
        # label = label[x:(new_img_shape[1] - y), x:(new_img_shape[1] - y)]
        ...
        # new_label[x:(img_shape[1] - y), x:(img_shape[1] - y)] = label
        return new_image
